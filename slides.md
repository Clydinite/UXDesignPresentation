...
<!--
接下來，我們進入第二個部分：設計方法。在這個章節中，我們將為各位展示我們如何運用「雙鑽石設計模型」中的「探索」與「定義」階段，從混沌的現象中梳理出清晰的脈絡。

我們並沒有一開始就決定要「做一個 App」。相反地，我們選擇先退後一步，讓數據和觀察說話。我們運用了田野調查 (Field Study)、同理心地圖 (Empathy Mapping) 以及聚類分析 (Clustering) 等方法，確保我們的每一個設計決策，都是建立在真實的使用者行為之上，而非我們自己的臆測。這是一個從「看到現象」到「理解本質」的過程。
-->
...
<!--
在專案初期，我們經常被問到：「為什麼不直接開始寫程式？」這讓我們反思了一個設計圈常見的謬誤——「德州神槍手謬誤 (Texas Sharpshooter Fallacy)」。也就是先隨便射一箭，再在彈孔周圍畫上靶心，宣稱自己百發百中。

在數位產品開發中，這就像是先決定了「我要做一個活動 App」，然後才拼命找理由說服自己使用者需要它。這種「先射箭再畫靶」的做法，往往只能解決表面的便利性問題（例如：活動列表不清楚），卻忽視了更深層的心理門檻（例如：使用者根本不敢參加活動）。

因此，我們堅持「先觀察再設計」。我們花費數週時間蹲點台南公園，不去預設解法，而是單純地記錄人們的行為。正是因為這個堅持，我們才觀察到了「數位孤島」這個關鍵現象——人們在公園裡，卻躲在手機螢幕後。這個發現成為了我們後續設計的核心錨點，也是我們與其他單純做「活動報名系統」的專案最大的不同之處。
-->
...
請看左上角的 U1，一個獨自騎車的孩子，他充滿好奇但在觀察；對比中間 U4 和 U5 的長者，他們有固定的儀式（如擦拭桌椅、玩牌），對空間有強烈的「主權感」。而像 U7、U8 這樣的學生族群，雖然身體在公園，但靈魂卻被吸進了手機裡。
...
第三部分 (第 12-18 行)： 這是最重要的一步：「去偏見化」。因為有些維度的數字天生就比較大，有些比較小，如果直接算，電腦會誤以為數字大的比較重要。這段程式碼就像一個「公平秤」，它把所有的分數縮放到同一個標準上，確保我們在分析「科技依賴」和「停留時間」時，這兩者的權重是一樣公平的。
-->
...
我們的設計機會就在這裡：我們發現公園裡存在一個巨大的「社交真空」。這群人其實都身處同一個空間，但因為彼此的「社交頻率」對不上，所以產生不了連結。特別是像 Group C 這種有穩定停留需求，卻又不知道如何跨出第一步的人。這讓我們確定了一件事：我們需要一個「社交緩衝層」，讓這些原本互不相干的泡泡，有機會溫柔地重疊在一起。
-->
...
我們從 12 個題目中精選了 4 個最重要的方向：

第一 (HMW 3 & 5)：我們思考如何設計一種「隱形的防護罩」。當她想滑手機逃避視線時，空間能不能給她一點隱私？ 第二 (HMW 11)：這是我們最核心的發想——「不需要說話的預約」。如果她能像訂位一樣，先在手機上定好那個讓她安心的角落，她就不會因為現場位置被佔走而感到慌張。 第三 (HMW 12)：我們要如何維持公園裡的「溫柔氛圍」？不是靠管理員趕人，而是透過設計，讓進入這個空間的人都有一種默契，不去打擾正在安靜相處的人。
...
接下來，我們要進入最令人興奮的部分：Part III，UX 設計解答。我們不想只丟出一堆功能清單，因為這不是在賣軟體。我們要用一個故事，一個屬於「楊子萱」的故事，來帶大家體驗這個設計是如何一步步解開她的心防，讓她從「不敢踏出門」，變成「期待每一次的相遇」。
